...
const Header = () => {
  ...
  return (
    ...
    >
      {menu && (
        <div onClick={() => setMenu(false)} className='fixed top-0 left-0 w-full h-full z-10'></div>
      )}
      <div className='max-w-7xl mx-auto md:flex-row flex-col flex text-center items-center justify-between gap-5 mb-3 w-full'>
        <Link to={'/dashboard'} className='font-bold text-2xl text-rose-500 md:mb-0 mb-2'>
          Administrador de Pacientes de <span className='text-gray-700'>Veterinaria</span>
        </Link>
        <div
          className={`${
            pathname === '/dashboard' ? 'grid-navegacion' : ' grid-cols-1 grid-rows-2'
        ...
        >
          ...
          >
            <input
              className={`md:w-[300px] w-full ml-auto border border-gray-100 outline-rose-100 md:focus-visible:w-[70%] transition-all outline-offset-0 p-2 rounded block text-gray-700 placeholder:text-gray-500 pr-10`}
              type='text'
              name='search'
              value={search}
            ...
            >
              <div className='text-gray-600 text-xl  py-2 block w-full hover:bg-slate-50 transition-colors'>
                <div className='flex items-center justify-center text-center gap-2 mb-1'>
                  ...
                  <p className='text-sm text-gray-500 font-semibold break-all'>{auth?.nombre}</p>
                </div>
                <a
                  onClick={() => setMenu(!menu)}
              ...
              <button
                ...
                className='flex items-center justify-center gap-2 text-white text-base font-bold bg-rose-500 hover:bg-rose-600 transition-colors py-2 w-full rounded-md'
              >
                <svg
                  xmlns='http://www.w3.org/2000/svg'
                  fill='none'
